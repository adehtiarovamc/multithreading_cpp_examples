...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < philosophers.size(); ++i) {
        philosophers[i] = std::thread([i, &cout_mtx, &fork_mtx, &mouthfulls]() {
            ...
            std::unique_lock<std::mutex> lfork_mtx(fork_mtx[lfork]);
            cout_lck.lock();
            std::cout << "Philosopher " << i << " takes left fork " << lfork << '\n';
            cout_lck.unlock();

            std::unique_lock<std::mutex> rfork_mtx(fork_mtx[rfork], std::defer_lock);
            bool rlocked = false;
            while (!rlocked) {
                cout_lck.lock();
                ...
                rlocked = rfork_mtx.try_lock();
                if (!rlocked) {
                    cout_lck.lock();
                    std::cout << "Philosopher " << i << " puts down right fork " << rfork << '\n';
                    cout_lck.unlock();
                    std::this_thread::sleep_for(1s);
                }
            }
